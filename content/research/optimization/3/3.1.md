...
9. **Relative Boundary:**{: style="color: SteelBlue  "}{: .bodyContents1 #bodyContents19}
    :   We can then define the relative boundary of a set $$\chi$$ as $$\mathbf{cl}  \chi \tex{ \ } \operatorname{relint} \chi,$$ where $$\mathbf{cl} \chi$$ is the closure of $$\chi$$.
...
3. **Cones [Cone Sets]:**{: style="color: SteelBlue  "}{: .bodyContents2 #bodyContents23}
    :   A set $$C$$ is a cone if $$x \in C$$, then $$\alpha x \in C$$, for every $$\alpja \geq 0$$. :   A set C is said to be a convex cone if it is convex and it is a cone.  
...
4. **Farkas lemma:**{: style="color: SteelBlue  "}{: .bodyContents6 #bodyContents64}
    :   Let $$A \in \mathbf{R}^{m \times n}$$ and $$y \in \mathbf{R}^m$$. Then, one and only one of the following two conditions is satisfied:  
        ...
        2. There exist $$z \in \mathbf{R}^m$$ such that $$z > A \geq 0, \: z^Ty < 0$$.
