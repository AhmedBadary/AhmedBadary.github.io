...
3. **Geometry of Hyperplanes:**{: style="color: SteelBlue  "}{: .bodyContents5 #bodyContents53}
    ...
    :   * **$$|b|$$** is, Precisely, the *length* of the *closest point* $$x_0$$ on $$\mathbf{H}$$ from the origin.
...
6. **Trace:**{: style="color: SteelBlue  "}{: .bodyContents8 #bodyContents86}
    :   The trace of a square $$n \times n$$ matrix $$A$$, denoted by $$\mbox{\bf Tr} A$$, is the sum of its diagonal elements:  
    :   $$ \mathbf{Tr} A = \sum_{i=1}^n A_{ii}$$.  
    * **Properties:**  
        * $$\mathbf{Tr} A = \mathbf{Tr} A^T$$.  
        * $$\mathbf{Tr} (AB) = \mathbf{Tr} (BA)$$.
        * $$\operatorname{trace}(XYZ) = \operatorname{trace}(ZXY).$$  
