...
//      throw new UnsupportedOperationException("hw3: TODO");
        this.upperbound = getNumRecordsPerPage();
        this.marked = false;
        this.markedentry = -1;
        this.nextRID = false;
...
//      throw new UnsupportedOperationException("hw3: TODO");
      ...
      for (int i = curInd + 1; i < getNumRecordsPerPage(); ++i) {
        byte[] bitmap = getBitMap(mypage);
        if (Bits.getBit(bitmap, i) == Bits.Bit.ONE) {
          this.nextRID = true;
          this.nextentry = i;
          return true;
...
//      throw new UnsupportedOperationException("hw3: TODO");
      if (this.hasNext()) {
        RecordId rid = new RecordId(mypage.getPageNum(), (short) nextentry);
        this.nextRID = false;
        this.nextcalled = true;
        this.curInd = this.nextentry;
        this.nextentry = 0;
        return rid;
      }
...
//      throw new UnsupportedOperationException("hw3: TODO");
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    ...
    boolean havecurrentPage;
    boolean havenextId;

    public RIDBlockIterator(BacktrackingIterator<Page> block) {
      this.block = block;
      this.havecurrentPage = false;
      this.havenextId = false;
...
//      throw new UnsupportedOperationException("hw3: TODO");
        ...
        if (this.havecurrentPage) {
          if (this.blockIter.hasNext()) {
            this.nextRecordId = this.blockIter.next();
            this.havenextId = true;
            return true;
          }
        }
        while (this.block.hasNext()) {
          this.havecurrentPage = true;
          this.currentPage = this.block.next();
          this.blockIter = new RIDPageIterator(this.currentPage);
        if (this.blockIter.hasNext()) {
          this.nextRecordId = this.blockIter.next();
          this.havenextId = true;
          return true;
