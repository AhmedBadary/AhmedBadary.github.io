...
5. **The State of the RNN $$\mathbf{h}^{(t)}$$:**{: style="color: SteelBlue"}{: .bodyContents1 #bodyContents15}  
    The network typically learns to use $$\mathbf{h}^{(t)}$$ as a kind of _lossy summary_ of the task-relevant aspects of the past sequencce of inputs up to $$t$$.  
...
6. **RNN Architectures/Design Patterns:**{: style="color: SteelBlue"}{: .bodyContents1 #bodyContents16}  
    ...
    ![img](/main_files/dl/archits/rnns/4.png){: width="100%"}  
        * __Architecture__:  
    ...
    ![img](/main_files/dl/archits/rnns/5.png){: width="100%"}  
        * __Architecture__:  
            * Produces an output at each time-step  
    ...
    ![img](/main_files/dl/archits/rnns/6.png){: width="100%"}  
        * __Architecture__:  
...
10. **Recurrent Networks as Directed Graphical Models:**{: style="color: SteelBlue"}{: .bodyContents1 #bodyContents110}  
    ...
    The introduction of _hidden state_ and _hidden-to-hidden_ connections can be motivated as reducing [fig 10.7]() to [fig 10.8](); which have $$\mathcal{O}(k^{\tau})$$ and $$\mathcal{O}(1)\*\tau$$ parameters, respectively (where $$\tau$$ is the length of the sequence).  
...
* __Drop in Recurrent Connections__: dropout is ineffective when applied to recurrent connections as repeated random masks zero all hidden units in the limit. The most common solution is to only apply dropout to non-recurrent connections.  
