...
4. **Faster R-CNNs:**{: style="color: SteelBlue"}{: .bodyContents3 #bodyContents34}  
    :   A single, end-to-end, architecture for object detection based on Fast R-CNNs, that tackles the bottleneck (of computing RoIs) in speed by introducing __Region Proposal Networks (RPNs)__ to make a CNN predict proposals from features.     
    :   * __Structure__:  
            * _Input_: Image vector  
            * _Output_: A vector of bounding boxes coordinates and a class prediction for each box     
    :   * __Key Insights__:  
