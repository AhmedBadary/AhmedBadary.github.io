...
2. **Formal Definition:**{: style="color: SteelBlue"}{: .bodyContents1 #bodyContents12}  
    ...
    * $$V(f(\vec{x}), y)$$: $$\:$$ an __error/loss function__  
    ...
    __Assumptions:__  
    * The training and test data are generated by an __unknown probability distribution over datasets__ (over the product space $$Z$$, denoted: $$p_{\text{data}}(z)=p(\vec{x}, y)$$) called the __data-generating process__.  
...
4. **Definitions:**{: style="color: SteelBlue"}{: .bodyContents1 #bodyContents14}  
    ...
    * __Generalization Gap:__{: style="color: red"}  
        ...
        __Computing the Generalization Error:__  
        Since $$I\left[f_{n}\right]$$ cannot be computed for an unknown distribution, the generalization error __cannot be computed__ either.  
    ...
    * __Excess Risk (Generalization-Gap) Decomposition \| Estimation-Approximation Tradeoff:__{: style="color: red"}  
        * [**Excess Risk Decomposition Video**](https://www.youtube.com/watch?v=YA_CE9jat4I&list=PLnZuxOufsXnvftwTB1HL6mel1V32w0ThI&index=5){: value="show" onclick="iframePopA(event)"}
        <a href="https://www.youtube.com/watch?v=YA_CE9jat4I&list=PLnZuxOufsXnvftwTB1HL6mel1V32w0ThI&index=5"></a>
            <div markdown="1"> </div>    
    <br>
...
1. **The Bias-Variance Decomposition:**{: style="color: SteelBlue"}{: .bodyContents3 #bodyContents31}    
    ...
    * __Bias:__ measures the expected deviation from the true value of the function or parameter.  
        > AKA: __Approximation Error__ (statistics)  How well can $$\mathcal{H}$$ approximate the target function '$$f$$'  
...
3. **Derivation:**{: style="color: SteelBlue"}{: .bodyContents3 #bodyContents33}  
    <p>$${\displaystyle {\begin{aligned}\operatorname {E} {\big [}(g^{(\mathcal{D})}-y)^{2}{\big ]}&=\operatorname {E} {\big [}(g^{(\mathcal{D})}- f -\varepsilon)^{2}{\big ]}\\&=\operatorname {E} {\big [}(f+\varepsilon -g^{(\mathcal{D})}+\bar{g}-\bar{g})^{2}{\big ]}\\&=\operatorname {E} {\big [}(\bar{g}-f)^{2}{\big ]}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(g^{(\mathcal{D})} - \bar{g})^{2}{\big ]}+2\operatorname {E} {\big [}(\bar{g}-f)\varepsilon {\big ]}+2\operatorname {E} {\big [}\varepsilon (g^{(\mathcal{D})}-\bar{g}){\big ]}+2\operatorname {E} {\big [}(g^{(\mathcal{D})} - \bar{g})(\bar{g}-f){\big ]}\\&=(\bar{g}-f)^{2}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(g^{(\mathcal{D})} - \bar{g})^{2}{\big ]}+2(\bar{g}-f)\operatorname {E} [\varepsilon ]+2\operatorname {E} [\varepsilon ]\operatorname {E} {\big [}g^{(\mathcal{D})}-\bar{g}{\big ]}+2\operatorname {E} {\big [}g^{(\mathcal{D})}-\bar{g}{\big ]}(\bar{g}-f)\\&=(\bar{g}-f)^{2}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(g^{(\mathcal{D})} - \bar{g})^{2}{\big ]}\\&=(\bar{g}-f)^{2}+\operatorname {Var} [y]+\operatorname {Var} {\big [}g^{(\mathcal{D})}{\big ]}\\&=\operatorname {Bias} [g^{(\mathcal{D})}]^{2}+\operatorname {Var} [y]+\operatorname {Var} {\big [}g^{(\mathcal{D})}{\big ]}\\&=\operatorname {Bias} [g^{(\mathcal{D})}]^{2}+\sigma ^{2}+\operatorname {Var} {\big [}g^{(\mathcal{D})}{\big ]}\\\end{aligned}}}$$</p>  
    where:  
    $$\overline{g}(\mathbf{x})=\operatorname{E}_ {\mathcal{D}}\left[g^{(\mathcal{D})}(\mathbf{x})\right]$$ is the __average hypothesis__ over all realization of $$N$$ data-points $$\mathcal{D}_ i$$.  

    <button>Derivation with Wikipedia Notation</button>{: .showText value="show"
     onclick="showText_withParent_PopHide(event);"}
    <p hidden="">$${\displaystyle {\begin{aligned}\operatorname {E} {\big [}(y-{\hat {f}})^{2}{\big ]}&=\operatorname {E} {\big [}(f+\varepsilon -{\hat {f}})^{2}{\big ]}\\&=\operatorname {E} {\big [}(f+\varepsilon -{\hat {f}}+\operatorname {E} [{\hat {f}}]-\operatorname {E} [{\hat {f}}])^{2}{\big ]}\\&=\operatorname {E} {\big [}(f-\operatorname {E} [{\hat {f}}])^{2}{\big ]}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(\operatorname {E} [{\hat {f}}]-{\hat {f}})^{2}{\big ]}+2\operatorname {E} {\big [}(f-\operatorname {E} [{\hat {f}}])\varepsilon {\big ]}+2\operatorname {E} {\big [}\varepsilon (\operatorname {E} [{\hat {f}}]-{\hat {f}}){\big ]}+2\operatorname {E} {\big [}(\operatorname {E} [{\hat {f}}]-{\hat {f}})(f-\operatorname {E} [{\hat {f}}]){\big ]}\\&=(f-\operatorname {E} [{\hat {f}}])^{2}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(\operatorname {E} [{\hat {f}}]-{\hat {f}})^{2}{\big ]}+2(f-\operatorname {E} [{\hat {f}}])\operatorname {E} [\varepsilon ]+2\operatorname {E} [\varepsilon ]\operatorname {E} {\big [}\operatorname {E} [{\hat {f}}]-{\hat {f}}{\big ]}+2\operatorname {E} {\big [}\operatorname {E} [{\hat {f}}]-{\hat {f}}{\big ]}(f-\operatorname {E} [{\hat {f}}])\\&=(f-\operatorname {E} [{\hat {f}}])^{2}+\operatorname {E} [\varepsilon ^{2}]+\operatorname {E} {\big [}(\operatorname {E} [{\hat {f}}]-{\hat {f}})^{2}{\big ]}\\&=(f-\operatorname {E} [{\hat {f}}])^{2}+\operatorname {Var} [y]+\operatorname {Var} {\big [}{\hat {f}}{\big ]}\\&=\operatorname {Bias} [{\hat {f}}]^{2}+\operatorname {Var} [y]+\operatorname {Var} {\big [}{\hat {f}}{\big ]}\\&=\operatorname {Bias} [{\hat {f}}]^{2}+\sigma ^{2}+\operatorname {Var} {\big [}{\hat {f}}{\big ]}\\\end{aligned}}}$$</p>
